...
function countAdjOccupiedSeats(row, col, prev_state) {
    ...
    for (const adj of adjs) {
        const adj_row = row + adj[0]
        const adj_col = col + adj[1]
        if (isValidPosition(adj_row, adj_col)) {
            const state = prev_state[adj_row][adj_col]
        ...
        for (let col = 0; col < states[row].length; col++) {
            ...
            switch(local_state) {
                case 'L':
                    ...
                    break
                case '#':
                    if (countAdjOccupiedSeats(row, col, prev_state) >= 4) {
                        states[row][col] = 'L'
                        changed = true
                    }
                    break
